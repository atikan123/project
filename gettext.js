...
const { PDFImage } = require("pdf-image"); // ใช้ pdf-image แทน pdf-lib
...
const extractImagesFromPDF = async (pdfBuffer) => {
    const pdfImage = new PDFImage(pdfBuffer);
    const imageFiles = [];
    ...
    for (const imagePath of imagePaths) {
        imageFiles.push(imagePath); // ใช้ path ของภาพที่แปลงแล้ว
    }
...
app.post('/', upload, async (req, res) => {
    ...
    try {
        if (!pdfFile && !imageFiles.length && !url && !divUrl) {
            return res.status(400).send('No files or URL uploaded');
        }

        // Handle URL for <h> and <p> tags
        if (url) {
        ...
        // Handle URL for <div> tags only
        if (divUrl) {
            const response = await fetch(divUrl);
            const html = await response.text();
            const $ = cheerio.load(html);
        
            const mainDivContent = $('main, .content, .article, .post-content, .entry-content, .section').first();
            const seenTexts = new Set();
        
            mainDivContent.find('div').each((i, elem) => {
                const divText = $(elem).text().trim().toLowerCase();
                
                if (divText && !seenTexts.has(divText)) { 
                    seenTexts.add(divText);
                    children.push(new Paragraph(divText));
                }
            });
        }                                                              
